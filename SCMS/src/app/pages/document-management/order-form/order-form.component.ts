    ...
    this.departments$.pipe(take(1)).subscribe({
      next: (departments) => {
        ...
        const toSafeString = (value: string | null | undefined): string =>
          (value ?? '').trim();
        const normalizeBoolean = (value: boolean | null | undefined): boolean =>
          value ?? false;

        const permissions = this.currentOrder
          ? {
              ...this.currentOrder.permissions,
              isPublic: normalizeBoolean(values.isPublic),
            }
          : {
        ...
        const baseOrder: Omit<Order, 'id' | 'createdAt' | 'updatedAt'> = {
          ...
          title: toSafeString(values.title),
          description: toSafeString(values.description),
